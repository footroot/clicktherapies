...
// This patch emulates Ruby It.it behaviour for JS I18n
var It = {
  selfClosingTags: ["br"],
  tag: function(tagName) {
    if(_.contains(It.selfClosingTags, tagName)) {
      return { start: "<" + tagName + " />", end: "" };
    } else {
      return { start: "<" + tagName + ">", end: "</" + tagName + ">" };
    }
  },
  link: function(url, props) {
    props = _.extend(props || {}, { href: url });
    var attr = _.map(props, function(val, key) {
      return key + "=\"" + val + "\"";
    }).join(" ");
    return { start: "<a " + attr + ">", end: "</a>" };
  },
  it: function(key, obj) {
    var str = I18n.t(key);
    _.each(obj, function(val, key) {
      var regexp = new RegExp("\\[missing %{" + key + ":?([^}]*)} value\\]", "g");
      str = str.replace(regexp, function(wholeString, content) {
        return val.start + content + val.end;
      });
    });
    return str;
  }
