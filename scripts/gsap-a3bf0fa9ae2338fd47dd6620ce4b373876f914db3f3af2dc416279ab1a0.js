...
/*jshint -W117 */ // suppress JSHint error codes

(function($) {

  // define editAni
  var editAni = function(container) {
    var $pencil = $(container).find('path#pencil'),
    $line = $(container).find('path#line'),
    editTl = new TimelineMax({
      repeat: -1,
      repeatDelay: 0.5
    });
    TweenLite.killTweensOf([$pencil, $line]);
    // editTl.timeScale(0.1);
 ...
 // invoke edit button pencil + line effect
 $('body').on('mouseenter', '.mbtn.edit', function () {
   editAni(this);
 });

 // define trash lid animation
 var trashLidAni = function (container, angle, yLid, yBin) {
   var $lid = $(container).find('path#lid'),
   $bin = $(container).find('path#bin');

   TweenLite.to($lid, 0.1, {
     rotation: angle,
     y: yLid,
     transformOrigin: 'left bottom',
     ease: Power1.ease
   });
   TweenLite.to($bin, 0.1, {
     y: yBin,
     ease: Power1.ease
   });
 };

 // invoke delete button lid effect
 $('body').on('mouseenter', '.mbtn.delete', function () {
   trashLidAni(this, -12, 0, 0.25);
 });
 $('body').on('mouseleave', '.mbtn.delete', function () {
   trashLidAni(this, 0, 0, 0);
 });

 // define add plus animation
 var addAni = function (container, angle, size) {
   var $plus = $(container).find('path');

   TweenLite.to($plus, 0.5, {
     rotation: angle,
     scale: size,
 ...
 // invoke add button pluss effect
 $('body').on('mouseenter', '.mbtn.add', function () {
   addAni(this, 180, -1);
 });
 $('body').on('mouseleave', '.mbtn.add', function () {
   addAni(this, 0, 1);
 });

 // define caret down animation
 var caretAni = function (container, angle, size, yVal) {
   var $caret = $(container).find('.svg_caret_down path');

   if ( $(container).parent().hasClass('dir-up') ) {
     TweenLite.to($caret, 0.2, {
       rotationY: angle,
       scaleY: size,
       y: yVal,
       transformOrigin: 'center',
 ...
 var caretAniDown = function (container, size) {
   var $caret = $(container).find('.svg_caret_down path');
 ...
 var caretAniClick = function (container, size) {
   var $caret = $(container).find('.svg_caret_down path');
 ...
 // invoke caret effect
 $('body').on('mouseenter', '.select .mbtn, .language_select .mbtn', function () {
   caretAni(this, 0, 0.85, 0);
 });
 $('body').on('mouseleave', '.select .mbtn, .language_select .mbtn', function () {
   caretAni(this, 0, 1, 0, 0);
 });

 $('body').on('mousedown', '.select .mbtn, .language_select .mbtn', function () {
   caretAniDown(this, 0.75);
 });

 $('body').on('click', '.select .mbtn, .language_select .mbtn', function () {
   caretAniClick(this, 1);
 });

 // define caretDirUp effect
 var caretDirUp = function (container, val, y) {
   var $caretSVG = $(container).find('.mbtn .svg_caret_down');

   TweenLite.to($caretSVG, 0.1, {
     svgOrigin: '7.5 0',
     y: y,
     scaleY: val
   });
 ...
 // invoke caretDirUp effect
 $('body').on('mouseenter', '.select.dir-up', function () {
   caretDirUp(this, -1, '-=5%');
   $('body').on('blur', '.select.dir-up', function () {
     caretDirUp(this, 1, '0%');
   });
 });
 $('body').on('mouseleave', '.select.dir-up', function () {
   if ( !$(this).children('.mbtn').is(':focus') ) {
     caretDirUp(this, 1, '0%');
   }
 });

 // define action cog spin animation
 var actionAni = function (container, duration, angle, ease) {
   var $cog = $(container).find('.svg_action path');

   TweenLite.to($cog, duration, {
     rotation: angle,
     transformOrigin: 'center',
     ease: ease
   });
 ...
 // invoke action / configure cog effect
 $('body').on('mouseenter', '.mbtn.action, .mbtn.configure, #madmimi_logo .mbtn.edit.simple', function () {
   actionAni(this, 10, 1440, Power0.easeNone);
 });
 $('body').on('mouseleave', '.mbtn.action, .mbtn.configure, #madmimi_logo .mbtn.edit.simple', function () {
   actionAni(this, 4, 0, Elastic.easeOut.config(1, 0.3));
 });

 // define stats effect
 var statsAni = function (container) {
   var $pie = $(container).find('.svg_pie_chart #pie'),
   $bars = [
     $(container).find('.svg_pie_chart #bar1'),
     $(container).find('.svg_pie_chart #bar2'),
     $(container).find('.svg_pie_chart #bar3')
   ],
   $peace0 = $(container).find('.svg_pie_chart #peace0'),
   $peace = $(container).find('.svg_pie_chart #peace');
   var statsTl = new TimelineMax({
     repeat: -1,
     repeatDelay: 0.5
   });
   TweenLite.killTweensOf([$pie, $bars, $peace, $peace0]);
   TweenLite.set([$peace, $peace0], { opacity: 0 });
   statsTl
   .set($pie, {
     morphSVG: '#pie',
     transformOrigin: 'center'
   ...
   .fromTo($pie, 0.8, {
     ...
     y: 0,
     xPercent: 0,
     ease: Elastic.easeOut.config(1.25, 0.6)
   })
   .to($pie, 0.5, {
     morphSVG: '#pie2',
     ease: Power2.easeIn
   ...
   .to($pie, 1, {
     morphSVG: '#pie',
     opacity: 1,
     ease: Elastic.easeOut.config(1.25, 0.8)
   });
 ...
 // invoke stats animation effect
 $('body').on('mouseenter', '.mbtn.simple.view_all_stats', function () {
   statsAni(this);
 });

 // invoke refresh animation effect
 $('body').on('mouseenter', '.mbtn.stats', function () {
   statsAni(this);
 });

 // define refresh effect
 var refreshAni = function (container) {
   var $arrows = $(container).find('.svg_refresh path'),
   $interval = 0.3,
   ease = Power2.ease,
   refreshTl = new TimelineMax({
    repeat: -1,
    repeatDelay: 0.5
   });
   refreshTl.to($arrows, $interval, {
     rotation: -25,
     ease: ease,
     transformOrigin: 'center'
   }).to($arrows, $interval, {
 ...
 // invoke refresh animation effect
 $('body').on('mouseenter', '.mbtn.refresh', function () {
   refreshAni(this);
 });

 // define clone effect
 var cloneAni = function (container) {
   var $sheep = $(container).find('.svg_clone g'),
   cloneTl = new TimelineMax({
     repeat: -1
   });
   cloneTl.to($sheep, 0.36, {
     yPercent: -20,
 ...
 // invoke clone animation effect
 $('body').on('mouseenter', '.mbtn.clone', function () {
   cloneAni(this);
 });

 // define image module animation
 var imageAni = function (container, x, y, sunScale, sunAlpha, nightAlpha, xMoon, yMoon, moonAlpha) {
   var $night = $(container).find('.svg_image path#night'),
      $sun = $(container).find('.svg_image circle#sun'),
      $moon = $(container).find('.svg_image circle#moon');

   TweenLite.to($sun, 0.45, {
     x: x,
     y: y,
     scale: sunScale,
     opacity: sunAlpha,
   ...
   TweenLite.to($night, 0.4, {
     opacity: nightAlpha,
     ease: Power1.easeInOut
   });

   TweenLite.to($moon, 0.5, {
     x: xMoon,
     y: yMoon,
     opacity: moonAlpha,
     ease: Power3.easeIn
 ...
 // invoke image animation effect
 $('body').on('mouseenter', '.mbtn.image', function () {
   imageAni(this, 1, 3, 0.25, 0, 0.25, -0.5, -1.25, 1);
 });
 $('body').on('mouseleave', '.mbtn.image', function () {
   imageAni(this, 0, 0, 1, 1, 0, 0, 0, 0);
 });

 var dt = new Date(),
 hours = dt.getHours(),
 minutes = dt.getMinutes(),
 _hours12 = (hours > 12) ? (hours-12) : (hours),
 rotationFinger = parseInt((minutes * 6)), // get angle for current minutes
 rotationThumb = parseInt((_hours12 * 30) + ((rotationFinger/360)*30)); // get angle for current hour

 // define schedule button clock face animation
var scheduleAni = function(container, ease, duration, thumb, finger) {
  var $finger = $(container).find('.svg_schedule path#finger'),
    $thumb = $(container).find('.svg_schedule path#thumb'),
    svgOrig = '8.5 8.5';

    TweenLite.killTweensOf([$finger, $thumb]);
...
// invoke schedule animation effect
$('body').on('mouseenter', '.mbtn.schedule', function() {
  scheduleAni(this, Power4.easeOut, 4, rotationThumb + 360, rotationFinger + 1800);
  // console.log(hours, minutes, _hours12, rotationThumb, typeof rotationThumb, rotationFinger, typeof rotationFinger);
});

$('body').on('mouseleave', '.mbtn.schedule', function() {
  scheduleAni(this, Power4.easeOut, 3, 50, 315);
});

 // define drag icon fx
 var dragAni = function (container, duration, scale, opacity, arrowEase) {
   var findDragElement = function(selector) {
     return $(container).find('.svg_drag ' + selector);
   };

   var $left  = findDragElement('#arrow_n'),
       $right = findDragElement('#arrow_s');

   TweenLite.to($left, duration, {
     transformOrigin: 'bottom',
     scaleY: scale,
     ease: arrowEase
   });

   TweenLite.to($right, duration, {
     transformOrigin: 'top',
     scaleY: scale,
     ease: arrowEase
   });
 ...
 // invoke drag button effect
 $('body').on('mouseenter', '.mbtn.drag', function () {
   dragAni(this, 0.5, 1.2, 0.25, Power2.easeOut);
 });

 $('body').on('mouseleave', '.mbtn.drag', function () {
   dragAni(this, 0.5, 1, 1, Power2.easeOut);
 });

 $('body').on('mousedown', '.mbtn.drag', function () {
   dragAni(this, 1, 1.3, 0, Elastic.easeOut.config(1.75, 0.4));
 });

 $('body').on('mouseup', '.mbtn.drag', function () {
   dragAni(this, 1, 1, 1, Elastic.easeOut.config(2.5, 0.3));
 });




 // invoke send button effect
 var sendAni = function (container) {
   var $mail = $(container).find('.svg_send #send');
   TweenLite.killTweensOf($mail);
   var sendTl = new TimelineMax({
     repeat: -1,
     repeatDelay: 0
   })
   .to($mail, 0.35, {
 ...
 $('body').on('mouseenter', '.mbtn.send', function () {
   sendAni(this);
 });


 // ****************** BUTTON FX *****************
 // invoke general btn material design-like effect
 var stage, el, circle, d, x, y;
 $('body').on('click', '.mbtn', function(e) {

   el = $(this);
   el.addClass('fx').append("<i class='stage'></i>");
   stage = el.find(".stage");

   stage.prepend("<span class='circle'></span>");
   circle = stage.find(".circle");


   if (!circle.height() && !circle.width()) {
     d = Math.max(stage.outerWidth(), stage.outerHeight());
     circle.css({
       height: d,
       width: d
     });
   ...
   y = e.pageY - stage.offset().top - circle.height() / 2;
   x = e.pageX - stage.offset().left - circle.width() / 2;
   ...
   TweenLite.to(circle, 0.5, {
     ...
     onComplete: removeStage
   });
 });

 // fancy mousedown event does break firefox - removing for now
 // var stage, el, circle, d, x, y;
 // $('body').on('mousedown', '.mbtn', function(e) {
 //
 //   el = $(this);
 //
 //   var removeStage = function () {
 //     el.find('.stage').remove();
 //     el.removeClass('fx');
 //     TweenLite.killTweensOf(el.find('.circle'));
 //   };
 //
 //   removeStage;
 //
 //   el.addClass('fx');
 //   if ( !el.find('.stage').length ) {
 //    el.append("<i class='stage'></i>");
 //   }
 //   stage = el.find('.stage');
 //
 ...
 // do stuff after Dashboard items have loaded
